...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Add to cart local storage state management
    cartOrderBtn.addEventListener('click', () => {
        let cart = JSON.parse(localStorage.getItem('mychronicle_cart')) || [];
        cart.push({ id: 'studio-book-' + Date.now(), title: 'Custom Designed Photobook', price: 3499, quantity: 1 });
        localStorage.setItem('mychronicle_cart', JSON.stringify(cart));
        
        cartOrderBtn.innerText = 'Added! Redirecting...';
        setTimeout(() => {
